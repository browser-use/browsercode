...
function BrowserExecute(props: ToolProps) {
  const { theme } = useTheme()
  const isRunning = createMemo(() => props.part.state.status === "running")
  const output = createMemo(() => stripAnsi(stringValue(props.metadata.output)?.trim() ?? ""))
  const [expanded, setExpanded] = createSignal(false)
  ...
  const prompted = createMemo(() => {
    const src = (stringValue(props.input.code) ?? "").replace(/^\n+|\n+$/g, "")
    if (!src) return ""
    return src
      .split("\n")
  ...
  return (
    <Switch>
      <Match when={props.metadata.output !== undefined}>
        <BlockTool
          title={`# ${stringValue(props.input.description) ?? "Browser execute"}`}
          part={props.part}
          spinner={isRunning()}
      ...
      <Match when={true}>
        <InlineTool icon=">" pending="Writing JS..." complete={stringValue(props.input.code)} part={props.part}>
          {prompted()}
        </InlineTool>
      </Match>
    </Switch>
